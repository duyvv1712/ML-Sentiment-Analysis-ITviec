...
*Cập nhật lần cuối: 04/09/2026 — Giai đoạn 1 & 2 hoàn thành (TV1 + TV2 = 50%) ✅ | Giai đoạn 3 (TV3) sẵn sàng nhận bàn giao ⏳*
...
| **3** | **Phân tích EDA & Trích xuất TF-IDF** | **TV2: Văn Duy** | ✅ **Hoàn thành** | Notebook `01_data_exploration_eda.ipynb` đã chạy đủ output; 9 biểu đồ 300 DPI tại `reports/figures/`; `src/features.py` + 11 test pass; Stratified 80/20 seed 2026 (development 6.730 / final test 1.683 khóa); artifacts `text_tfidf_vectorizer.joblib`, `text_feature_extractor.joblib`, `train_test_features.joblib`, `artifact_manifest.json` (có checksum + runtime). CV development: TF-IDF (1,2) Macro F1 **0,5597**. Báo cáo: `reports/eda_feature_engineering.md`. |
